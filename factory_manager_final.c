...
int main (int argc, const char * argv[] ){
	...
	/* Now, declare auxiliary variables*/
		...
		if (atoi(aux)!=0){
			...
			if(k!=0){
				printf("[ERROR][factory_manager] Invalid file k\n"); /* Error message */
				exit(-1);
			}
		}
		...
		for (i = 0; i<j; i++){       /* It goes for each process_manager to be created */
			for(k = 0; k < 4; k++){  /* It goes through all the variables of a given process_manager */
				if(k == 1){
					...
					strcpy(aux_acc[k+1],semName); /*  semaphore's name */
				}
				else{ /* The rest of the arguments */
					int length;
					if ((length=snprintf(NULL,0,"%d",param[i][k]))<0){ /* We calculate the length of the integer */
						printf("[ERROR][factory_manager] Process_manager with id %i has finished with errors sn1.\n", param[i][0]);
						return -1;
					}
					...
					if((auxiliar=malloc (length +1))==NULL){ 
						printf("[ERROR][factory_manager] Process_manager with id %i has finished with errors mal1.\n", param[i][0]);
						return -1;
					}
					if((aux_acc[k+1] = malloc (length +1))==NULL){
						printf("[ERROR][factory_manager] Process_manager with id %i has finished with errors mal2.\n", param[i][0]);
					...
					if((snprintf(auxiliar,length + 1,"%d",param[i][k]))<0){ /* Convert the int into a string */
						printf("[ERROR][factory_manager] Process_manager with id %i has finished with errors sn2.\n", param[i][0]);
						return -1;
					}
			...
			switch(pid){
				...
				printf("[ERROR][factory_manager] Process_manager with id %i has finished with errors fork.\n", param[i][0]);
				return -1;
				
				case 0: /* Child process, the one which executes process_manager */
				if(execvp("/home/100346094/Escritorio/lab3/process",aux_acc)<0) {  // Execute process_manager 
					printf("[ERROR][factory_manager] Process_manager with id %i has finished with errors exec.\n", param[i][0]); // Error message                        				
					return -1;
					}
				...
				*/
				default: // The parent waits until the children has ended 
					while (wait(&status) != pid) {
						...
						if (status != 0) {
							printf("[ERROR][factory_manager] Process_manager with id %i has finished with errors status.\n", param[i][0]); // Error message 
							return -1;
						}
					}
