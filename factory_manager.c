...
/*
  It is a method in charge of writing the errors in the
	standard error
*/
...
int ThrowError_ProcessManager(char* id){
	char *aux1 = "[ERROR][factory_manager] Process_manager with id ";
	char *aux2 = " has finished with errors.\n";  /* Error message */
	int aux_length = strlen(aux1); //We calculate the length of the total message
	aux_length += strlen(aux2);
	aux_length += strlen(id);
	char *error = malloc(aux_length+1);
	strcpy(error, aux1);
	strcat(error,id);
	strcat(error,aux2);
	if (write(2,error,strlen(error))<0){
